defmodule PasskeysWeb.UserLive.Settings do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.table id="credentials" rows={@credentials}>
        <:col :let={credential} label="Credential id">{inspect(credential.id)}</:col>
        <:col :let={credential} label="Public key">
          {UserCredential.cose_key_to_string(credential)}
        </:col>
        <:col :let={credential} label="Authenticator">
          {icon_for_authenticator(credential.aaguid)}
        </:col>
      </.table>
    </Layouts.app>
    """
  ...
  def handle_info({:credential_created, _credential}, socket) do
    user = socket.assigns.current_scope.user
    socket = assign(socket, :credentials, Accounts.credentials_for_user_id(user.id))
    {:noreply, socket}
  end
  ...
  defp icon_for_authenticator(aaguid) do
    case Wax.Metadata.get_by_aaguid(aaguid) do
      {:ok, metadata} ->
        icon = metadata["icon"] || @unknown_icon
        model = metadata["description"] || "Unknown: #{inspect(metadata)}"

        Phoenix.HTML.raw(~s|<img class="max-w-10" src="#{icon}"/><br/>#{model}|)

      {:error, _} ->
        icon_for_authenticator(aaguid, false)
    end
  end

  defp icon_for_authenticator(aaguid, false) do
    Phoenix.HTML.raw(
      ~s|<img class="max-w-10" src="#{@unknown_icon}"/><br/>Unknown: no metadata for #{inspect(aaguid)}|
    )
  end
end
