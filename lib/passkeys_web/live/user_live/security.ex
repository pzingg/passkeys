defmodule PasskeysWeb.UserLive.Security do
    ...
    <Layouts.app flash={@flash} width_class="w-2/3" current_scope={@current_scope}>
      ...
      >
        <:col :let={{_id, credential}} label="Credential id">
          {String.slice(credential.id, 1..16) <> "..."}
        </:col>
        ...
        <:col :let={{_id, credential}} label="Authenticator">
          <.authenticator_icon metadata={fido2_metadata(credential)} />
        </:col>
      ...
      <div id="create-passkey" phx-hook="register_passkey">
        <.button variant="primary" phx-click="create_passkey" phx-value-attestation="none">
          Create passkey (software; none)
        </.button>
        <.button phx-click="create_passkey" phx-value-attestation="direct">
          Create passkey (hardware; direct)
        </.button>
      </div>
    </Layouts.app>
    """
  ...
  @impl true
  def handle_event("create_passkey", %{"attestation" => attestation}, socket) do
    user = socket.assigns.current_scope.user
    challenge = Wax.new_registration_challenge(attestation: attestation)
  ...
  def handle_event(
    ...
    socket =
      case Accounts.register_user_credential(
             ...
             socket.assigns.webauthn_challenge,
             raw_id_b64,
  ...
  @impl true
  def handle_info({:credential_created, credential}, socket) do
    credential = maybe_with_td_class(credential)
    socket = stream_insert(socket, :credentials, credential)
  ...
  def authenticator_icon(assigns) do
    ~H"""
    <div><img class="max-w-10" src={Map.fetch!(@metadata, "icon")} /></div>
    <div>{Map.fetch!(@metadata, "description")}</div>
    """
  end

  defp fido2_metadata(%{aaguid: nil}) do
    %{"description" => "No aaguid provided", "icon" => @unknown_icon}
  end

  defp fido2_metadata(%{aaguid: aaguid}) do
    metadata =
